...
async function fetchLiveForex() {
    try {
        ...
        if(usdEl) usdEl.innerHTML = 'USD/INR <span class="forex-up">84.10 ▲</span>'; 
        const eurEl = document.getElementById('fx-eur');
        if(eurEl) eurEl.innerHTML = 'EUR/INR <span class="forex-up">90.50 ▲</span>';
    }
}
...
function initSite() {
    ...
    const observer = new IntersectionObserver((entries) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('active'); if (entry.target.classList.contains('timeline-wrapper')) { const pl = document.getElementById('progressLine'); if(pl) pl.style.width = '100%'; } } }); }, { threshold: 0.15 }); 
    ...
    const hubX = 500; const hubY = 325; const nodeCoords = [{x:100,y:50}, {x:900,y:50}, {x:50,y:310}, {x:950,y:310}, {x:100,y:580}, {x:900,y:580}];
    
    if(netContainer && svgLayer) { 
        networkNodes.forEach((node, index) => { 
            const div = document.createElement('div'); div.className = `export-node node-${index} reveal`; 
...
// Hero Slider Variables (Global)
let slideIndex = 0; 
let slideTimer;

function setSlide(n) { slideIndex = n; updateSlider(); resetTimer(); } 
...
// Start Slider immediately if elements exist, otherwise wait for init
